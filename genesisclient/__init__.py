...
def download(client, args):
    ...
    if args.regionalschluessel is not None and args.regionalschluessel != '*':
        ...
        path = '%s_%s.%s' % (
            args.download, args.regionalschluessel, args.format)
...
def search(client, args):
    ...
    for cat in list(result['meta'].keys()):
        if result['meta'][cat] > 0:
            print(("Hits of type '%s': %d" % (
                cat.upper(), result['meta'][cat])))
    for hit in result['results']:
        otype = hit['type'].upper()
        if otype == 'MERKMAL' or otype == 'STATISTIK':
            print(("%s %s %s" %
                  (otype, clean(hit['name']), clean(hit['description']))))
        elif otype == 'TABELLE':
            print(("%s %s %s" %
                  (otype, clean(hit['name']), clean(hit['description']))))
        elif otype == 'BEGRIFF':
